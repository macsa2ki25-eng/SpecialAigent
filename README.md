...
## 併設ツール: ポケカ 優勝デッキまとめ (シティリーグ / ジムバトル)

上記の投稿エージェントとは独立して動く、子ども向けのツール (APIキー不要)。
シティリーグとジムバトルの結果から優勝・準優勝デッキだけを毎日自動で集め、
8歳の子どもが一人で読めるページを GitHub Pages に公開する。

```bash
python -m src.pokeca.cli collect   # 収集
python -m src.pokeca.cli build     # 子ども向けページを生成
python -m src.pokeca.cli rank      # 直近1週間の優勝ランキング
```

詳細は [docs/POKECA_GUIDE.md](./docs/POKECA_GUIDE.md) 参照。
